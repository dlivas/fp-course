    ...
    . filtering
      (\a ->
        (StateT
          (\s ->
            if a <= 100
              then Full (a `S.notMember` s, a `S.insert` s)
              else Empty)))
...
instance Monad f => Monad (OptionalT f) where
  (=<<) ::
    ...
    -> OptionalT f b
  f =<< (OptionalT m) =
    OptionalT $ onFull (runOptionalT . f) =<< m

...
hex ::
  Parser Char
hex =
  chr . hex2Int <$> replicateA 4 hexParser
  where
    hexParser =
      satisfy isHexDigit
    hex2Int h =
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll predicates =
  satisfy (and . sequence predicates)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny predicates =
  satisfy (or . sequence predicates)

    ...
    if (length l) == (length r')
      then
        ...
        let
          (a'' :. l'') = l'
        in
          IsZ (ListZipper l'' a'' (reverse r' ++ (a :. r)))
    ...
    if (length r) == (length l')
      then
        ...
        let
          (a'' :. r'') = r'
        in
          IsZ (ListZipper (reverse l' ++ (a :. l)) a'' r'')
...
moveLeft (ListZipper Nil _ _) =
  IsNotZ
moveLeft (ListZipper (a' :. l) a r) =
    IsZ (ListZipper l a' (a :. r))
...
moveRight (ListZipper _ _ Nil) =
  IsNotZ
moveRight (ListZipper l a (a' :. r)) =
    IsZ (ListZipper (a :. l) a' r)
...
moveLeftN n lza
  | n == 0 =
    IsZ lza
  | n < 0 =
    moveRightN (negate n) lza
  | otherwise =
      case (moveLeft lza) of
        IsNotZ -> IsNotZ
        IsZ lza' -> moveLeftN (n - 1) lza'
...
moveRightN n lza
  | n == 0 =
    IsZ lza
  | n < 0 =
    moveLeftN (negate n) lza
  | otherwise =
      case (moveRight lza) of
        IsNotZ -> IsNotZ
        IsZ lza' -> moveRightN (n - 1) lza'
...
moveLeftN' n lza@(ListZipper l _ _)
  | n < 0 =
    moveRightN' (negate n) lza
  | n > lengthl =
    Left lengthl
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n lza@(ListZipper _ _ r)
  | n < 0 =
    moveLeftN' (negate n) lza
  | n > lengthr =
    Left lengthr
...
deletePullLeft (ListZipper Nil _ _) =
  IsNotZ
deletePullLeft (ListZipper (a :. l) _ r) =
  IsZ $ ListZipper l a r
...
deletePullRight (ListZipper _ _ Nil) =
  IsNotZ
deletePullRight (ListZipper l _ (a :. r)) =
  IsZ $ ListZipper l a r
...
instance Extend ListZipper where
  ...
  f <<= lz =
    ListZipper
      (zipperList moveLeft lz)
      (f lz)
      (zipperList moveRight lz)
    where
      zipperList moveX =
        unfoldr $ ((\z -> (f z, z)) <$>) . toOptional . moveX
    -- error "todo: Course.ListZipper (<<=)#instance ListZipper"
